...
```bash
python evaluate.py run prompts/v2.yaml          # evaluate, store, auto-diff
python evaluate.py run prompts/v2.yaml --limit 5 --skip-judge   # cheap smoke run
python evaluate.py runs                         # list recorded runs
python evaluate.py diff <base_run_id> <head_run_id>
python -m pytest                                # 79 tests, no API calls
```
...
models.py  gemini.py              contract and transport
```

A run is `(prompt version, model, dataset version, judge version, timestamp)` plus one scored result per case, stored in SQLite with the constraints enforced by the database rather than only in Python.
...
**Confounders are detected.** If two runs differ in model, dataset version or judge version, the diff prints `NOT A CLEAN PROMPT COMPARISON` and names what changed. This was added after a diff proudly reported a 20% improvement that was entirely due to a model swap.
...
Full runs on 2026-08-25, `gemini-3.5-flash-lite`, all 94 cases:
...
- **Free-tier quota constrains iteration.** A full judged run costs 188 requests against a 500/day cap.
...
promptwatch/
  ...
  gemini.py              API key and cached client
